    ...
    <div className="space-y-6">
      ...
      {latestVersion && (
        <div className="rounded-lg border border-glass-border bg-surface p-4 space-y-3">
          ...
          <PermissionBadges permissions={latestVersion.permissions as Record<string, string> | null} />
          ...
          {latestVersion.fileSpecs && (latestVersion.fileSpecs as Array<{ path: string }>).length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {(latestVersion.fileSpecs as Array<{ path: string }>).map((f) => (
                  <code key={f.path} className="rounded bg-surface-elevated px-2 py-0.5 font-mono text-xs text-neon-cyan-dim">
                    {f.path}
                  </code>
                ))}
              </div>
