...
export async function GET(req: NextRequest) {
  ...
  if (!token || token.length !== 64) {
    return new NextResponse(htmlPage("Invalid Link", "This unsubscribe link is invalid.", false), {
      status: 400,
  ...
  if (!subscriber) {
    return new NextResponse(
      htmlPage("Invalid Link", "This unsubscribe link is invalid or has expired.", false),
  ...
  if (subscriber.status === "UNSUBSCRIBED") {
    return new NextResponse(
      htmlPage("Already Unsubscribed", "You've already been unsubscribed.", true),
  ...
  return new NextResponse(
    htmlPage(
      "Unsubscribed",
      "You've been unsubscribed from DojOps updates. You won't receive any more emails.",
      true,
    ),
    { status: 200, headers: { "Content-Type": "text/html; charset=utf-8" } },
  );
